# severity_score_model_v2_enhanced.py
"""
Enhanced Severity Score Model v2 - Production Ready
===================================================

Advanced Features:
- Non-destructive operations (reads Final.csv only)
- Balanced severity distribution via oversampling
- Feature selection by importance (top K sensors)
- Stratified K-Fold cross-validation
- Hyperparameter tuning (GridSearchCV)
- Stacking ensemble (GB + RF + meta-learner)
- Comprehensive evaluation and diagnostics
- Future-ready for multi-fault analysis
...
TOP_K_FEATURES = 50           # Top sensor features (increased for better coverage)
CV_SPLITS = 5

# Optimized hyperparameter grids based on previous results
GB_PARAM_GRID = {
...
def load_and_prepare_data():
    ...
    print("🎯 ENHANCED SEVERITY SCORE MODEL V2")
    print("="*50)
...
# ==========================================================
def train_enhanced_models(X_selected, y_balanced, y_binned_balanced):
    """Train models with GridSearchCV and create stacking ensemble."""
    print(f"\n🤖 Training enhanced models with GridSearchCV...")
    ...
    # Create stratified splits manually for regression
    ...
    # Use Ridge regression as meta-learner (more stable than GB)
    meta_learner = Ridge(alpha=1.0)

    # Use simple 5-fold CV for stacking (not stratified to avoid issues)
    ...
    # Evaluate stacking with cross-validation using simple CV
...
# ==========================================================
def save_models_and_results(results, selected_features, importances, evaluation, X_original):
    """Save all models, scalers, and results."""
    print(f"\n💾 Saving models and results to {OUTPUT_DIR}...")
    
    # Save models
    joblib.dump(results["stacking_model"], os.path.join(OUTPUT_DIR, "severity_stacking_model_v2.pkl"))
    joblib.dump(results["scaler"], os.path.join(OUTPUT_DIR, "severity_scaler_v2.pkl"))
    
    # Save selected features
    pd.Series(selected_features).to_csv(
        os.path.join(OUTPUT_DIR, "selected_features_v2.csv"), index=False, header=["feature"]
    )
    
    # Save feature importances
    importances.to_csv(os.path.join(OUTPUT_DIR, "feature_importances_v2.csv"))
    ...
    pd.DataFrame(summary).to_csv(os.path.join(OUTPUT_DIR, "model_v2_summary.csv"), index=False)
...
if __name__ == "__main__":
    try:
        ...
        results = train_enhanced_models(X_selected, y_balanced, y_balanced_binned)

        # 5. Evaluate on original data
        evaluation = evaluate_final_model(results, X_original, y_original, selected_features)

        # 6. Save everything
        summary = save_models_and_results(results, selected_features, importances, evaluation, X_original)
