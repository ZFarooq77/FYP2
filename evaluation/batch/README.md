...
| ------------------ | ---- | ------ | ------ | ----- | ------------ |
| **Severity Score** | 0.96 | 0.015  | 0.0225 | 2.19% | 🟢 Excellent |
| **TTF (km)**       | 0.98 | 184.16 | 724.41 | 0.44% | 🟢 Excellent |
...
- Monitor **Severity Score** on future datasets to ensure R² and MAE remain stable.
- If performance degrades, retrain using the full grid search by setting `FAST_MODE = False` in `modules/train_severity_score.py`.
...
The improved severity_score model used by this batch evaluator is trained via `modules/train_severity_score.py`.
...
After retraining the model, re-run `python evaluate_batch.py` in this folder to refresh the metrics and plots.
